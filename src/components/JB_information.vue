<template>
  <div class="container" style="padding-top: 56px">
    ...
    <div class="row">
      <h3 style="text-align: center">安裝原理</h3>
      <p>{{ installmsg }}</p>
      <div class="col-sm-4">
        <img :src="JBimg" class="img-thumbnail border-0 rounded" />
      </div>
      <div class="col-sm-8">
    ...
    <h3 class="text-center">機器開機方式</h3>
    <div class="row">
      <div v-for="item of jb_difference">
        <h4>{{ item.title }}</h4>
        <p>{{ item.content }}</p>
      </div>
      <div class="col-sm-6 text-center">
        <img class="rounded" :src='JBimg' />
        <p>螢幕底部會有一個跑條向右跑，如未按下音量小聲鍵則會自動進入虛擬系統</p>
      </div>
      <div class="col-sm-6 text-center">
        <img class="rounded" :src='JBimg' />
        <p>進入開機選單後的畫面</p>
    ...
    <div class="row" style="padding-top: 30px">
      <div v-for="item in startoptions" class="row">
        <div class="col">
...
import { ref } from "vue"
const JBimg = new URL('https://fakeimg.pl/400x300/200').href
const optionimg = new URL('https://fakeimg.pl/150x150/200').href
...
const startoptions = ref([
  { id: id++, title: '正版系統(無破解)', content: '該系統為本機原有的正版系統，如要使用Nintendo Switch Online服務(例如動物森友會或是魔物獵人)，則必須使用該系統才能享受原本Switch主機連線的樂趣喔。點選後會讀取機器本身記憶模塊的正版系統，所有功能都跟未改機之前是一模一樣的，但相簿功能加載了額外的所有插件。' },
